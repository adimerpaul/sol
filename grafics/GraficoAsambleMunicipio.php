...
FROM recinto r2 INNER JOIN mesa m ON m.idrecinto =r2.idrecinto 
WHERE r2.idmunicipio =$idmunicipio
AND r2.idrecinto IN (SELECT idrecinto FROM votacion v WHERE v.idtipocandidatura=3)");
$c=0;
while($fd=mysqli_fetch_array($cn))
{
    $cantidadvotada=$fd[0];
    ...
    $(function () {
        $(document).ready(function () {
            ...
            $('#dona').highcharts({
                ...
                title: {
                    text: 'TOTAL ACUMULADO CANDIDATO ASAMBLEISTA TERRITORIO EN ESTE MUNICIPIO AL <?=number_format ($porcentaje,2)?> %'
                },
                tooltip: {
